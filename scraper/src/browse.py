...
def ask_user():
    ...
    """
    while True:
        response = color_input("Docs folder already exists. Do you want to overwrite it, ignore and continue, or cancel the operation? (overwrite/ignore/cancel): ", "yellow").strip().lower()
        ...
        if response in ('overwrite', 'o'):
            ...
            print("Invalid input. Please enter 'overwrite' (o), 'ignore' (i), or 'cancel' (c).")
...
if os.path.exists(DOCS_ROOT):
    ...
    if user_choice == 'overwrite':
        ...
        color_print("Existing docs folder will be ignored. Continuing operation.", "yellow")
...
def browse_screen(screen, project, session):
    ...
    if all(map(lambda file_name: os.path.exists(os.path.join(screen_json_folder, file_name)), file_names)):
        color_print(f"   ⮑  Screen {screen['name']} data already exists locally. Skipping.", 'yellow')
...
def browse_project(project, session):
    ...
    if details:
        ...
        with ThreadPoolExecutor(max_workers=min(5, os.cpu_count())) as executor:
            future_to_screen_id = {executor.submit(browse_screen, screen, project, session): screen['id'] for screen in details.get('screens', [])}
            ...
            for future in as_completed(future_to_screen_id):
                ...
                try:
                    ...
                    color_print(f'   ✘  Screen {screen_id} generated an exception: {exc}', 'red')
